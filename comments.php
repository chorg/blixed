...
<?php if ($comments) : ?>
	...
	<h2><?php comments_number('没有评论', '1 条评论', '% 条评论' ); if($post->comment_status == "open") { ?> <a href="#commentform" class="more">添加你的评论</a><?php } ?></h2>
  ...
  <p class="avatarimg"><?php if(function_exists('get_avatar')) echo get_avatar(get_comment_author_email(), '45') ;?></p>
	<?php comment_text() ?>
...
<?php else : ?>
			...
			<p><label for="author">昵称</label> <input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" tabindex="1" /> <?php if ($req) echo "<em>必填</em>"; ?></p>
			<p><label for="email">邮箱</label> <input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" tabindex="2" /> <em><?php if ($req) echo "Required, "; ?>保密</em></p>
			<p><label for="url">网址</label> <input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" tabindex="3" /></p>
